...
## Convert the Given Indices to Position and Vice Versa
...
## Move the Given Player by the Given Value
...
## Generate Snakes and Ladders in the Grid
...
## Get the Movement Value of the Given Position
...
int get_entity_value(int p) {
    int plus_value  = get_ladder_plus_value(p);
    int minus_value = get_snake_minus_value(p);
    return abs(plus_value) > abs(minus_value)? plus_value : minus_value;
}
```
...
void check_ladder_state(int player, int entity_value) {
    if (entity_value > 0) {
        //Prints the grid
        print_grid();
...
void check_snake_state(int player, int entity_value) {
    if (entity_value < 0) {
        //Prints the grid
        print_grid();
...
void play_game() {
    ...
    while (true) {
        ...
        //Check if the state of the grid has a ladder or a snake
        check_ladder_state(player, entity_value);
        check_snake_state(player, entity_value);
